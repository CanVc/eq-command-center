...
Inventory dumps generated by `/outputfile inventory` are imported automatically while the API is running. The API looks for the latest `<Character>_<server>-Inventory.txt` files in the EverQuest root inferred from the configured log path (`.../EverQuest/Logs/eqlog_*.txt` -> `.../EverQuest`). You can override that directory with `EQMARKET_INVENTORY_DIR`. Unchanged dumps are skipped by hash, and changed dumps replace the current character inventory while keeping import history.

Manual inventory import remains available:

```bash
eqmarket import-inventory --db data/eqmarket.sqlite --file "C:\\Users\\Public\\Daybreak Game Company\\Installed Games\\EverQuest\\Dreadnought_frostreaver-Inventory.txt"
```
